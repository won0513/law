...
@bp.route('/precedent/<string:c1>/<string:c2>')
...
def search_index():
    ...
    if opt != 3:
        ...
        a_list.append(pd.read_pickle("/var/www/myapp/src/lawarticle_4_label.pkl"))
    ...
    if opt != 2:
        ...
        try:
            ...
            word = session["word"]
            if word != content: #기존에 검색했던 단어와 다를 경우 word와 name_list, last_num 삭제
                session.pop("word", None)
                session.pop("num_list", None)
                ...
                p_dic = set_n_dic(name_list, last_num, page, word)
                session["page"] = page ##session storage에 page 저장
...
#    return render_template('search.html', p_dic=p_dic, a_dic=a_dic, opt=opt, opt_list=opt_list)
    ...
    #return render_template('search.html', p_dic=p_dic, a_dic=a_dic, opt_list =opt_list, opt=opt)
    return json.dumps(data, ensure_ascii=False)
@bp.route('/search/article/<string:c>')
def search_article(c):
    ...
    return json.dumps(data, ensure_ascii=False)
@bp.route('/search/precedent/<string:c>')
...
@bp.route('/precedent/detail/<string:num>')
def show_precedent(num):
    ...
    return json.dumps(data, ensure_ascii=False)
@bp.route('/article/<string:c1>/<int:c2>')
...
def kobert():
    ...
    r = predict('A와 B가 C에게 돈을 빌려주면서 C의 아파트에 근저당권 설정등기를 하려고 합니다. A와 B 각각의 명의로 돈을 낸 비율에 따라 동순위의 근저당권을 설정하고 싶은데 가능한가요?')

    def get_document_vectors(document_list):
        document_embedding_list = []
        loaded_model = FastText.load("/var/www/myapp/src/law/pan_20000_fst_kk_2")  # 모델 로드
        # 각 문서에 대해서
...
@bp.route('/pan', methods=('GET', 'POST'))
def pan():
    query = 0 ##사용자 입력 문장(없으면 0)
    r = 0
    label = 0
    t1, t2 = 0, 0
    pos1 = '/var/www/myapp/src/law/pan_'
    pos2 = '/var/www/myapp/src/law/pan_'
    pan_list = []
    if request.method == 'POST':
        ...
        pan = pd.read_pickle(pos2 + str(label) + '_han_kk_label.pkl')
        pan2 = pd.read_pickle(pos1 + str(label) + '.pkl')
